"""
Benchmark voor galgje-agents
============================
Voert alle agents uit op testwoorden en vergelijkt prestaties.

Metingen:
    - Winrate (%)
    - Gemiddeld aantal foute gokken per game
    - Gemiddeld totaal aantal gokken per game
"""
...
def load_word_list(path: str = "words.txt") -> list[str]:
    """Laad woorden uit words.txt."""
    words = []

    paths = [path]
    if path == "words.txt":
        paths.append("word.txt")
    ...
    if not words:
        raise FileNotFoundError(
            "Geen woordenlijst gevonden. Verwacht words.txt."
        )
...
def run_game(agent: BaseAgent, secret_word: str, word_list: list[str],
    ...
    """Speel een galgje-game met deze agent. Geef resultaat-dict terug."""
...
def benchmark(n_games: int = 200, max_wrong: int = 6, seed: int = 42,
              verbose_games: int = 3):
    """Draai alle agents op dezelfde set woorden en print resultaten."""
    random.seed(seed)
    ...
    # Sorteer op winrate (hoog naar laag)
    ranking = []
